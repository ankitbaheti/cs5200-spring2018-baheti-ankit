...
public class DeveloperDAO extends BaseDAO implements DeveloperDAOInterface {
    ...
    public void changeNumber(String developerName, String phoneNumber){
        try {
            Class.forName(JDBC_DRIVER);
            Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
            String sql = "update phones set phone = ? and person = (select id from person where username = ?) and `primary` = 1;";
            PreparedStatement pstm = conn.prepareStatement(sql);
            pstm.setString(1, phoneNumber);
            pstm.setString(2, developerName);
            pstm.executeUpdate();
            pstm.close();
            conn.close();
        } catch (ClassNotFoundException | SQLException e) {
    ...
    public void deletePrimaryAddress(String developerName){
        try {
            Class.forName(JDBC_DRIVER);
            Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
            String sql = "delete from address where person = (select id from person where username = ?) and `primary` = 1;";
            PreparedStatement pstm = conn.prepareStatement(sql);
            pstm.setString(1, developerName);
            pstm.executeUpdate();
            pstm.close();
            conn.close();
        } catch (ClassNotFoundException | SQLException e) {
